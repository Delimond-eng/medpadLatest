...
class _PatientRegisterState extends State<PatientRegister> {
  ...
  final textProfession = TextEditingController();
  String province = "";
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          children: [
                              ...
                              children: [
                                Flexible(
                                  child: InputText(
                                      title: "Profession",
                                      hintText:
                                          "Entrez la profession du patient",
                                      icon: CupertinoIcons
                                          .person_2_square_stack_fill,
                                      inputController: textProfession,
                                      keyType: TextInputType.text,
                                      isRequired: true),
                                ),
                                SizedBox(
                                ...
                                Flexible(
                                  child: InputText(
                                      title: "Téléphone",
                                      hintText:
                                          "Entrez le n° de contact du patient",
                                      icon: Icons.phone,
                            ...
                            InputText(
                                title: "Email",
                                hintText: "Entrez l'email du patient",
                                icon: Icons.email,
  ...
  Future<void> savePatient() async {
    ...
    for (int i = 0; i < fieldArr.length; i++) {
      if (fieldArr[i] == "") {
        XDialog.showErrorMessage(context,
            ...
            message:
                "Vous devez entrer toutes les infos requises du patient !");
        return;
      }
    }
    ...
    if (strB64Image1.isEmpty || strB64Image2.isEmpty || strB64Image3.isEmpty) {
      XDialog.showErrorMessage(context,
          ...
          message: "vous devez enroller les empreintes du patient",
          color: bgColor);
    ...
    try {
      ...
      await ApiManagerService.savePatient(
              nom: GxdCryptor.encrypt(textNom.text),
              profession: GxdCryptor.encrypt(textProfession.text),
              adresse: GxdCryptor.encrypt(textAdresse.text),
              ville: textVille.text,
              email: GxdCryptor.encrypt(textEmail.text),
              nais: GxdCryptor.encrypt(textDateNais.text),
              sexe: sexe,
              tel: GxdCryptor.encrypt(textPhone.text),
        ...
        Xloading.dismiss();
        if (result != null) {
          storage.write("patient_name", textNom.text);
          XDialog.showSuccessAnimation(context);
          List<String> currentPatientList = [
            textNom.text,
            textDateNais.text,
            textProfession.text,
            sexe,
          ...
          appController.singlePatientDocList.value = currentPatientList;
          cleanFields();
        } else {
  ...
  void cleanFields() async {
    ...
    Future.delayed(Duration(milliseconds: 500), () {
      Navigator.push(
          context, MaterialPageRoute(builder: (context) => PatientTabPage()));
    });
  }
}
